...
def plot_unit(bit, y0, x0, width, height):
    if (bit==0):
        ...
        vlines(x0, y0, y0+height, color='c', linewidth = 2)
        hlines(y0+height, x0, x0+width)
        vlines(x0+width, y0, y0+height, color='c', linewidth = 2)
        time = linspace(x0,x0+width,50)
        fill_between(time, y0, y0+height, facecolor='c')

def sig_plot(clk, x_start, y_start):
    x0 = x_start;y0=y_start
    for i in range(len(clk)):
        plot_unit(clk[i],y0,x0, 1, 5)
        x0 = x0 + 1
...
clock =  [1,0,0,0,0,1,1,0,1,0,1,1]

x = linspace(-0.5,11, 500)
##clk = piecewise(x, [x<0,(x>=0)*(x<1), (x>=1)*(x<2), (x>=2)*(x<3), (x>=3)*(x<4), x>=4],[4,5,4,5,4,5])
...
##y3 = piecewise(x, [x<0.3,(x>=0.3)*(x<1.3), (x>=1.3)*(x<2.3), (x>=2.3)*(x<3.3), (x>=3.3)*(x<4.3), x>=4.3],[0.5,-0.5,0.5,-0.5,0.5,-0.5])
##output = (y3+0.5)*(clk-4)-2
...
hlines(5, -0.5, 11, colors='red', linestyle='--')
hlines(5, -0.5, 11, colors='red', linestyle='--')
##hlines(2.5, -0.5, 5, colors='black', linestyle='--')
##hlines(1, -0.5, 5, colors='black', linestyle='--')
##hlines(-0.5, -0.5, 5, colors='black', linestyle='--')
##hlines(-2, -0.5, 5, colors='black', linestyle='--')
#fill_between(x, -3, 3, facecolor=(1,0,0,0.5))
sig_plot(clock, -0.5, 0);
...
text(0.5, 5.1, 'Start', family='serif', style='italic', size=13)
text(9.5, 5.1, 'Stop', family='serif', style='italic', size=13) 
#plot_unit(1, 2, 0, 1, 1)
...
##xlim(-1,6)
ylim(-1,7)
title('Serial communication ATMega - letter "X"')
savefig('es5_letterX_atmega.png', dpi=400)
show()
