...
data = genfromtxt('V_ripetuti_-1+1_21mis_rgen')
...
text(xdata.max()*0.1, ydata.max()*0.7, chiquadro, family='serif', style='italic', size=15) 
text(xdata.max()*0.1, ydata.max()*0.81, degfreedom, family='serif', style='italic', size=14)
text(xdata.max()*0.1, ydata.max()*0.6, pendenza, family='serif', style='italic', size=15)
text(xdata.max()*0.1, ydata.max()*0.47, intercetta, family='serif', style='italic', size=15)

title(r"Amplificatore invertente fit - $R_{1} = R_{G} = 2.2 k\Omega$") 

ax = axes()
savefig('fit_V_ripetuti_21mis_rgen.png', dpi=400) 
show()
