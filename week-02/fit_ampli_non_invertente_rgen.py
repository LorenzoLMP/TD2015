...
data = genfromtxt('non-inv_0_8-21_rgen_2mo')
...
vout2 = []
for i in range(len(vout)-2):
    vout2.append(vout[i+2])
vout2 = array(vout2)
...
p0 = [-5.9 , 0.1] #metti il valore inizialeee
...
pendenza = r'$V_{out}/V_{in}:  %s  \pm  %s $' %(round(p[0],2),round(var[0],2))
intercetta = r'Offset: $ %s  \pm  %s [V]$' %(round(p[1],3),round(var[1],3))

text(xdata.min()*0.9, ydata.max()*0.80, chiquadro, family='serif', style='italic', size=15) 
text(xdata.min()*0.9, ydata.max()*1, degfreedom, family='serif', style='italic', size=14)
text(xdata.min()*0.9, ydata.max()*0.60, pendenza, family='serif', style='italic', size=15)
text(xdata.min()*0.9, ydata.max()*0.40, intercetta, family='serif', style='italic', size=15)

title(r"Amplificatore non-invertente fit - $R_{G} = 2.2 M\Omega$") 

ax = axes()
savefig('fit_non-invertente_rgen_2mo.png', dpi=400) 
show()
