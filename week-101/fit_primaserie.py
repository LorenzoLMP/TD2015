...
data = genfromtxt('prima_serie.txt')
...
sigma = sqrt(sigmay**2 + (0.3*sigmax/c)**2)
############################################################

#Parte per il fit

def fitfunc(x, *par): 
    return (7.32 + (par[1] - 1)*x)/c + par[0]

p0 = [5, 1.5] #metti il valore inizialeee

p, pcov = optimize.curve_fit(fitfunc, xdata, ydata, p0, sigma)
...
text(xdata.min()*1, 34, degfreedom, family='serif', style='italic', size=15)
text(xdata.min()*1, 33.7, pendenza, family='serif', style='italic', size=15)
text(xdata.min()*1, 33.4, intercetta, family='serif', style='italic', size=15)

title(r"Indice rifrazione acqua - prima serie $K = 7.32(2)$ [m]")
