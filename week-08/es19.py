...
data = genfromtxt('es19_30mis')

xdata = data[:,0]
ydata = data[:,1]

##data1 = genfromtxt('es17_360ohm')
##xdata1 = data1[:,0]
##ydata1 = data1[:,1]
#zdata = sfasa
#sigmay = ydata1/100
...
plot(xdata, ydata, linestyle="None",marker=".", color="blue", markersize= 10)
title(r"Corrente LED $I_{F}$ in funzione di $V_{F}$", size = 15)
#errorbar(xdata, ydata, sigmay, sigmax, linestyle="None", color="red")
#errorbar(xdata1, ydata1, sigmay, sigmax, linestyle="None", color="red")
#xscale('log')
#xlim(0.08,4)
#ylim(0.9,10)
#xscale('log')
#yscale('log')
xlabel(r'Tensione $V_{F}$ [V]')
ylabel(r'Corrente $I_{F}$ [mA]')
grid('on', which = "both")
#minorticks_on()
